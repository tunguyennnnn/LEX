...
import (
	...
	"github.com/lex/queueclient/internal/assets"
)
...
func NewServer(namespace string, pool *redis.Pool, hostPort string) *Server {
	...
	router.Middleware(func(c *context, rw web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
		c.Server = server
		c.Enqueuer = work.NewEnqueuer("work", server.pool)
		next(rw, r)
	})
	router.Middleware(func(c *context, rw web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	...
	router.Post("/enqueue", (*context).enqueue)
...
func (c *context) enqueue(rw web.ResponseWriter, r *web.Request) {
	id := r.Request.URL.Query().Get("id")
	if id == "" {
		render(rw, nil, errors.New("Id is missing from query parameters"))
	}
	job, err := c.Enqueuer.Enqueue("job", work.Q{"id": id})
	response := struct {
