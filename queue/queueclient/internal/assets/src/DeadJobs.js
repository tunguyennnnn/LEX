...
export default class DeadJobs extends React.Component {
  ...
  fetch() {
    ...
    request(`${this.props.fetchURL}?page=${this.state.page}`, 'GET').
      then((resp) => resp.json()).
      then((data) => {
        this.setState({
          selected: [],
  ...
  deleteAll() {
    ...
    request(`${this.props.deleteAllURL}`, 'POST')
      .then(() => {
        this.updatePage(1);
      });
  }
  ...
  deleteSelected() {
    ...
    this.state.selected.map((job) => {
      ...
      p.push(request(`${this.props.deleteURL}/${job.died_at}/${job.id}`, 'POST'));
    });
  ...
  retryAll() {
    ...
    request(this.props.retryAllURL, 'POST').then(() => {
      this.updatePage(1);
    });
  }
  ...
  retrySelected() {
    ...
    this.state.selected.map((job) => {
      ...
      p.push(request(`${this.props.retryURL}/${job.died_at}/${job.id}`, 'POST'));
    });
